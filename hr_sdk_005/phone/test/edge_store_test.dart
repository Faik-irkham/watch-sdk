...
void main() {
  ...
  test('kiriman tersimpan, kiriman ulang tidak menggandakan data', () async {
    expect(
      await store.insertBatch('heart_rate', [heartRate(1), heartRate(2)]),
      2,
    );
    expect(
      await store.insertBatch('heart_rate', [heartRate(2), heartRate(3)]),
      1,
    );
  ...
  test('baris tidak lengkap membatalkan seluruh kiriman', () async {
    await expectLater(
  ...
  test('tabel yang tidak dikenal ditolak', () async {
    await expectLater(
      store.insertBatch('ppg', [heartRate(1)]),
      throwsArgumentError,
    );
  });

  test('ringkasan: jumlah, baris terbaru, dan waktu terima terakhir', () async {
    await store.insertBatch('spo2', [
      {
        'id': 1,
        'measured_at': 5000,
        'spo2_percent': 98,
    ...
    await store.insertBatch('accelerometer', [
      for (var i = 1; i <= 3; i++)
